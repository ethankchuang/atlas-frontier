...
async def upload_image_to_supabase(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[Image Storage] StorageException during upload:")
            ...
            logger.error(f"[Image Storage] Upload failed - Type: {type(upload_error).__name__}, Error: {upload_error}")
        ...
        # Handle Supabase storage-specific errors
        logger.error(f"[Image Storage] StorageException uploading image to Supabase:")
        logger.error(f"  - Name: {getattr(e, 'name', 'N/A')}")
        logger.error(f"  - Message: {getattr(e, 'message', str(e))}")
        logger.error(f"  - Code: {getattr(e, 'code', 'N/A')}")
        logger.error(f"  - Status: {getattr(e, 'status', 'N/A')}")
        ...
        error_type = type(e).__name__
        error_msg = str(e) if str(e) else "No error message"
        logger.error(f"[Image Storage] Error uploading image to Supabase - Type: {error_type}, Message: {error_msg}")
        if hasattr(e, '__dict__'):
            logger.error(f"[Image Storage] Error details: {e.__dict__}")
        import traceback
